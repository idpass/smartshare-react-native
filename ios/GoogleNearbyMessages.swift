...
class NearbyMessages: RCTEventEmitter {
  ...
  enum GoogleNearbyMessagesError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
      switch self {
      case .permissionError(let permissionName):
        return
          "Permission has been denied! Denied Permission: \(permissionName). Make sure to include NSBluetoothPeripheralUsageDescription in your Info.plist!"
  ...
  func connect(
    _ apiKey: String, discoveryModes: [NSString], discoveryMediums: [NSString],
    ...
    self.messageManager = GNSMessageManager(
      ...
      paramsBlock: { (params: GNSMessageManagerParams?) in
        ...
        params.microphonePermissionErrorHandler = { (hasError: Bool) in
          if hasError {
            self.sendEvent(
              withName: EventType.PERMISSION_ERROR.rawValue,
        ...
        params.bluetoothPowerErrorHandler = { (hasError: Bool) in
          if hasError {
            self.sendEvent(
              withName: EventType.BLUETOOTH_ERROR.rawValue,
              body: ["message": "Bluetooth is powered off/unavailable!"])
          }
        }
        params.bluetoothPermissionErrorHandler = { (hasError: Bool) in
          if hasError {
            self.sendEvent(
              withName: EventType.PERMISSION_ERROR.rawValue,
              body: ["message": "Bluetooth Permission denied!"])
          }
        }
  ...
  func publish(
    _ message: String, resolver resolve: RCTPromiseResolveBlock,
    rejecter reject: RCTPromiseRejectBlock
  ) {
    print("GNM_BLE: Publishing...")
    do {
      if self.messageManager == nil {
        throw GoogleNearbyMessagesError.runtimeError(
          message: "Google Nearby Messages is not connected! Call connect() before any other calls."
        )
      }
      self.currentPublication = self.messageManager!.publication(
        with: GNSMessage(content: message.data(using: .utf8)),
  ...
  func unpublish(_ resolve: RCTPromiseResolveBlock, rejecter reject: RCTPromiseRejectBlock) {
  ...
  func subscribe(_ resolve: RCTPromiseResolveBlock, rejecter reject: RCTPromiseRejectBlock) {
      ...
      self.currentSubscription = self.messageManager!.subscription(
        messageFoundHandler: { (message: GNSMessage?) in
          guard let data = message?.content else {
            self.sendEvent(
              withName: EventType.MESSAGE_NO_DATA_ERROR.rawValue,
              body: ["message": "Message does not have any Data!"])
            return
          }
          print("GNM_BLE: Found message!")
          self.sendEvent(
            withName: EventType.MESSAGE_FOUND.rawValue,
            body: ["message": String(data: data, encoding: .utf8)])
        },
        messageLostHandler: { (message: GNSMessage?) in
          ...
          self.sendEvent(
            withName: EventType.MESSAGE_LOST.rawValue,
            body: ["message": String(data: data, encoding: .utf8)])
        },
  ...
  func checkBluetoothAvailability(
    ...
    if self.tempBluetoothManager != nil || self.tempBluetoothManagerDelegate != nil {
      let error = GoogleNearbyMessagesError.runtimeError(
        message: "Another Bluetooth availability check is already in progress!")
    ...
    tempBluetoothManagerDelegate = BluetoothManagerDelegate(
      resolver: resolve, parentReference: self)
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(10)) {
      if !self.didCallback {
        ...
        let error = GoogleNearbyMessagesError.runtimeError(
          message:
            "The CBCentralManager (Bluetooth) did not power on after 10 seconds. Cancelled execution."
        )
  ...
  func parseDiscoveryMediums(_ discoveryMediums: [NSString]) -> GNSDiscoveryMediums {
  ...
  func parseDiscoveryModes(_ discoveryModes: [NSString]) -> GNSDiscoveryMode {
